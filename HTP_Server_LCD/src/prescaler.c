...
void calculate_prescaler(Energy_Modes EMx, float time_period)
{
...
uint32_t calculate_COMP0_Count(Energy_Modes EMx, float time_period)
{
	int COMP0_Count = 0;
	if (EMx < 3)
	{
...
uint32_t calculate_COMP1_Count(Energy_Modes EMx, uint32_t comp0_count,
		float on_time)
{
	int COMP1_Count = 0;
	if (EMx < 3)
